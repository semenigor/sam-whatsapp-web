...
async function samDecryptChooseFileManual() {
  const button = document.getElementById(SAM_DECRYPT_BUTTON_ID);
  ...
  try {
    ...
    samEncryptShowStatus('SAM Encrypt: вибери .samenc файл для розшифрування...');

    const result = await ipcRenderer.invoke('sam-encrypt:choose-file-and-decrypt', {
      revealInFolder: true
    });

    if (result && result.cancelled) {
      samEncryptShowStatus('SAM Encrypt: вибір .samenc скасовано.');
      return;
    }

    if (!result || !result.ok) {
      samEncryptShowStatus(`SAM Encrypt: помилка розшифрування: ${result && result.error ? result.error : 'невідома помилка'}`, true);
      console.error('SAM Decrypt result:', result);
      return;
    }

    samEncryptShowStatus('SAM Encrypt: файл розшифровано. Відкрито Finder з результатом.');
    console.log('SAM Encrypt decrypted file:', result);
  } catch (error) {
    samEncryptShowStatus(`SAM Encrypt: помилка розшифрування: ${error.message || error}`, true);
    console.error('SAM Decrypt button error:', error);
  } finally {
    if (button) {
      button.disabled = false;
      button.style.opacity = '1';
...
function samDecryptEnsureButton() {
  if (!SAM_DECRYPT_BUTTON_ENABLED) {
    const existing = document.getElementById(SAM_DECRYPT_BUTTON_ID);
    if (existing) {
      existing.remove();
    }
    return;
  ...
  if (document.getElementById(SAM_DECRYPT_BUTTON_ID)) {
    return;
  }

  const button = document.createElement('button');
  button.id = SAM_DECRYPT_BUTTON_ID;
  button.type = 'button';
  button.textContent = 'SAM 🔓';
  button.title = 'SAM Encrypt: розшифрувати отриманий .samenc файл';
  button.style.position = 'fixed';
  button.style.left = '5px';
  button.style.top = '349px';
  ...
  button.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    samDecryptChooseFileManual();
  });

  document.body.appendChild(button);
}

function samDecryptStartButton() {
  samDecryptEnsureButton();
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', samDecryptStartButton, { once: true });
} else {
